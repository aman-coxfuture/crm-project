...
export default function FeesPage() {
  ...
  const reloadData = async () => {
    try {
      ...
      const [feeResponse, paymentResponse] = await Promise.all([
        feeService.getSchoolFees(),
        feeService.getSchoolPayments(),
      ]);

      if (!feeResponse.success) {
        throw new Error(feeResponse.message || "Failed to load fees");
      }

      if (!paymentResponse.success) {
        throw new Error(paymentResponse.message || "Failed to load payments");
      }

      const mappedFees = (feeResponse.fees || []).map((fee) => ({
        ...fee,
        id: fee._id,
        studentId: fee.studentId?._id || fee.studentId || "",
        studentName: fee.studentId?.name || "",
        studentEmail: fee.studentId?.email || "",
        rollNumber: fee.studentId?.admissionNumber || "",
        class: fee.className || "",
        section: fee.section || "",
      ...
      setStudentLedgers(mappedFees);
      const mappedPayments = (paymentResponse.payments || []).map(
        (payment) => ({
          ...payment,
          id: payment._id,
          ...
          rollNumber: payment.studentId?.admissionNumber || "",
      ...
      setFeeTransactions(mappedPayments);
    } catch (err) {
      console.error("Failed to load fee data:", err);
      toastError(err.message || "Failed to load fee data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredLedgers = useMemo(() => {
    return studentLedgers.filter((item) => {
      ...
      const matchSession =
        !selectedSession || item.academicSession === selectedSession;

      return (
        matchSearch && matchClass && matchSection && matchStatus && matchSession
      );
    });
  }, [
    studentLedgers,
  ...
  const filteredFineRecords = useMemo(() => {
    return studentLedgers
      .filter((item) => {
        ...
        const matchSection =
          fineSection === "All" || item.section === fineSection;
        const matchSession =
          !fineSession || item.academicSession === fineSession;

        let matchStatus = true;
        if (fineStatus === "Paid") matchStatus = item.status === "PAID";
        else if (fineStatus === "Pending")
          matchStatus = item.status === "PENDING";
        else if (fineStatus === "Partially Paid")
          matchStatus = item.status === "PARTIAL";
        else if (fineStatus === "Overdue")
          matchStatus = item.status === "OVERDUE";
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      percentages: {
        ...
        partial: totalStudents
          ? Math.round((partialCount / totalStudents) * 100)
          : 0,
        pending: totalStudents
          ? Math.round((pendingCount / totalStudents) * 100)
          : 0,
        overdue: totalStudents
  ...
  const collectionMetrics = useMemo(() => {
    ...
    return {
      todayAmount: getAmount(startOfToday),
      todayCount: getCount(startOfToday),

      weekAmount: getAmount(startOfWeek),
      weekCount: getCount(startOfWeek),
  ...
  const classOverviewData = useMemo(() => {
    ...
    const pendingAmt = classStudents.reduce(
  ...
  const handleRecordPaymentSubmit = async (e) => {
    ...
    try {
      ...
      if (!paymentModalData?.id) {
        toastError("Fee record not found");
        return;
      }
      ...
      const payload = {
        studentId:
          paymentModalData.studentId?._id || paymentModalData.studentId,

        feeId:
          paymentModalData.feeId || paymentModalData._id || paymentModalData.id,
      ...
      const response = await feeService.createPayment(payload);
      ...
      setSelectedReceiptForPrint(response.payment);

      success(
        `Payment of ${formatCurrency(amount)} recorded successfully! Receipt: ${response.payment.receiptNo}`,
      );
    } catch (err) {
      console.error("Failed to record fee payment:", err);

      toastError(err.message || "Failed to record fee payment");
    }
  };
  ...
  return (
    <div style={{ paddingBottom: "40px" }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <div>
              ...
              >
                Class-wise, section-wise fee ledgers, automated late fines,
                collection receipts & audits
              </p>
            </div>
          </div>
        </div>
          ...
          <button
            ...
            onClick={() => {
              const defaultStudent =
                studentLedgers.find((s) => s.status !== "PAID") ||
                studentLedgers[0];
      ...
      >
        {[
          ...
          { key: "collection", label: "Fee Collection", icon: TrendingUp },
          {
            ...
            label: "Pending Fees",
            icon: AlertCircle,
            badge: metrics.counts.pending + metrics.counts.partial,
          },
          {
            key: "overdue",
            label: "Overdue Fees",
            icon: AlertTriangle,
            badge: metrics.counts.overdue,
            badgeColor: "#ef4444",
          },
          {
            key: "late-fine",
            label: "Late Fine",
            icon: ShieldAlert,
            badge: fineMetrics.studentsWithFine,
            badgeColor: "#8b5cf6",
          },
                ...
                <span
                  style={{
                    backgroundColor: isActive
                      ...
                      : tab.badgeColor || "rgba(99,102,241,0.15)",
                    color: isActive
                      ? "#ffffff"
                      : tab.badgeColor
                        ? "#ffffff"
      ...
      {activeTab === "dashboard" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div className="grid-4" style={{ gap: "16px" }}>
            <div
              className="card"
              style={{ padding: "18px", borderLeft: "4px solid #6366f1" }}
            >
              <div
                style={{
                  fontSize: "0.85rem",
            ...
            <div
              className="card"
              style={{ padding: "18px", borderLeft: "4px solid #3b82f6" }}
            >
              <div
                style={{
                  fontSize: "0.85rem",
            ...
            <div
              className="card"
              style={{ padding: "18px", borderLeft: "4px solid #10b981" }}
            >
              <div
                style={{
                  fontSize: "0.85rem",
              ...
              >
                {metrics.totalFees
                  ? Math.round((metrics.collected / metrics.totalFees) * 100)
                  : 0}
                % Realized
              </div>
            </div>
            ...
            >
              ...
              >
                {metrics.counts.pending + metrics.counts.partial} Students with
                balance
              </div>
            </div>
          </div>
          ...
          <div className="grid-2" style={{ gap: "16px" }}>
            <div
              className="card"
              style={{ padding: "18px", borderLeft: "4px solid #ef4444" }}
            >
              <div
                style={{
                  display: "flex",
            ...
            <div
              className="card"
              style={{ padding: "18px", borderLeft: "4px solid #8b5cf6" }}
            >
              <div
                style={{
                  display: "flex",
          ...
          <div className="card" style={{ padding: "20px" }}>
            ...
            <div className="grid-4" style={{ gap: "16px" }}>
              ...
              >
                ...
                >
                  {formatCurrency(metrics.amounts.paid)} (
                  {metrics.percentages.paid}%)
                </div>
              </div>
              ...
              >
                ...
                >
                  {formatCurrency(metrics.amounts.partial)} (
                  {metrics.percentages.partial}%)
                </div>
              </div>
              ...
              >
                ...
                >
                  {formatCurrency(metrics.amounts.pending)} (
                  {metrics.percentages.pending}%)
                </div>
              </div>
              ...
              >
                ...
                >
                  {formatCurrency(metrics.amounts.overdue)} (
                  {metrics.percentages.overdue}%)
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: "20px" }}>
            ...
            >
              <div>
                ...
                >
                  Select any class from Nursery to Class 10 to inspect cohort
                  metrics
                </p>
              </div>
            ...
            >
              ...
              <div className="grid-4" style={{ gap: "12px" }}>
                ...
                >
                  ...
                  >
                    Paid: {classOverviewData.paid} | Partial:{" "}
                    {classOverviewData.partial}
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    Collection:{" "}
                    {classOverviewData.totalFee
                      ? Math.round(
                          (classOverviewData.collected /
                            classOverviewData.totalFee) *
                            100,
                        )
                      : 0}
          ...
          >
            ...
            >
              <span>
                ...
                {studentLedgers.length} student ledgers
              </span>
              <span>
                Academic Session: <strong>{selectedSession}</strong>
              </span>
            </div>
          </div>
                ...
                <tbody>
                    ...
                    <tr key={item.id}>
                      <td style={{ textAlign: 'left' }}>
                        <div>
                          ...
                          >
                            ID: {item.studentId} • Parent: {item.parentName} (
                            {item.parentPhone})
                          </div>
                        </div>
                      </td>
                  ...
                  {filteredLedgers.length === 0 && (
                    <tr>
                      ...
                      >
                        No student fee records found matching your active
      ...
      {activeTab === "collection" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div className="grid-4" style={{ gap: "16px" }}>
            <div
              className="card"
              style={{ padding: "18px", borderTop: "4px solid #10b981" }}
            >
              <div
                style={{
                  fontSize: "0.8rem",
              ...
              >
                {formatCurrency(collectionMetrics.todayAmount)}
              </div>
              <div
                style={{
                  fontSize: "0.8rem",
              ...
              >
                {collectionMetrics.todayCount} Transactions cleared
              </div>
            </div>

            <div
              className="card"
              style={{ padding: "18px", borderTop: "4px solid #3b82f6" }}
            >
              <div
                style={{
                  fontSize: "0.8rem",
              ...
              >
                {formatCurrency(collectionMetrics.weekAmount)}
              </div>
              <div
                style={{
                  fontSize: "0.8rem",
              ...
              >
                {collectionMetrics.weekCount} Transactions
              </div>
            </div>

            <div
              className="card"
              style={{ padding: "18px", borderTop: "4px solid #6366f1" }}
            >
              <div
                style={{
                  fontSize: "0.8rem",
              ...
              >
                {formatCurrency(collectionMetrics.monthAmount)}
              </div>
              <div
                style={{
                  fontSize: "0.8rem",
            ...
            <div
              className="card"
              style={{ padding: "18px", borderTop: "4px solid #8b5cf6" }}
            >
              <div
                style={{
                  fontSize: "0.8rem",
          ...
          <div className="card" style={{ padding: "20px" }}>
            ...
            <div className="grid-4" style={{ gap: "16px" }}>
              ...
              >
                ...
                >
                  {formatCurrency(paymentMethodMetrics.upi.amount)}
                </div>
                <div
                  style={{
                    fontSize: "0.8rem",
                ...
                >
                  {paymentMethodMetrics.upi.count} txns
                </div>
              </div>
              ...
              >
                ...
                >
                  {formatCurrency(paymentMethodMetrics.bankTransfer.amount)}
                </div>
                <div
                  style={{
                    fontSize: "0.8rem",
              ...
              >
                ...
                >
                  {formatCurrency(paymentMethodMetrics.cash.amount)}
                </div>
                <div
                  style={{
                    fontSize: "0.8rem",
              ...
              >
                ...
                >
                  {formatCurrency(paymentMethodMetrics.cheque.amount)}
                </div>
                <div
                  style={{
                    fontSize: "0.8rem",
                  ...
                  {feeTransactions.slice(0, 5).map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>
                        ...
                        >
                          Class {t.class} - Sec {t.section}
                        </div>
                      </td>
                      <td>{t.feeType || t.feeHead}</td>
                      <td>
                        <span className="badge badge-info">
                          {t.paymentMethod || t.mode}
                        </span>
                      </td>
                      <td
                        style={{
                    ...
                    .map((item) => (
                      <tr key={item.id}>
                        <td>
                          ...
                          >
                            ID: {item.studentId} • Parent: {item.parentPhone}
                          </div>
                        </td>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          >
            <div>
              ...
              >
                Accounts where due dates have elapsed. Late fines are calculated
                deterministically with grace periods.
              </p>
            </div>
            ...
            >
              <table className="custom-table">
                ...
                <tbody>
                    ...
                    .map((item) => (
                      ...
                      >
                        <td>
                          ...
                          >
                            ID: {item.studentId} • Roll: {item.rollNumber} • Ph:{" "}
                            {item.parentPhone}
                          </div>
                        </td>
                        <td>
                  ...
                  {studentLedgers.filter((s) => s.status === "OVERDUE")
                    .length === 0 && (
                    <tr>
                      <td
                        colSpan={9}
                      ...
                      >
                        ✓ All student fee accounts are currently within due
                        dates or fully settled!
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {activeTab === "late-fine" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          >
            ...
            >
              <span>
                Showing <strong>{filteredFineRecords.length}</strong> student
                late fine accounts
              </span>
              <span>
                Current Policy:{" "}
                <strong>
                  {fineSettings.fineType === "fixed"
                    ? `₹${fineSettings.fixedAmount || 500} Fixed`
                    : `₹${fineSettings.fineAmount || 50}/day`}
                </strong>{" "}
                (Grace: {fineSettings.gracePeriod} days)
              </span>
            </div>
          </div>
                ...
                <tbody>
                  {filteredFineRecords.map((item) => (
                    <tr
                      ...
                      style={{
                        borderLeft:
                          item.fineAmount > 0 ? "4px solid #8b5cf6" : undefined,
                      }}
                    >
                      <td>
                        <div>
          ...
          >
            <div>
              <h3 style={{ fontSize: "1.2rem", fontWeight: 800, margin: 0 }}>
                Fee Payment History & Receipt Ledger
              </h3>
              <p
                style={{
                  fontSize: "0.9rem",
              ...
              >
                Auditable log of all processed student fee payments, receipts
                and vouchers
              </p>
            </div>
          </div>
                  ...
                  {feeTransactions.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>
                        ...
                        >
                          ID: {t.studentId} • Roll: {t.rollNumber}
                        </div>
                      </td>
                      <td>
                        <span className="badge badge-primary">
            ...
            >
              <div>
                <h3 style={{ fontSize: "1.15rem", fontWeight: 800, margin: 0 }}>
                  Class-Wise Fee Collection Report (K-10)
                </h3>
                <p
                  style={{
                    fontSize: "0.85rem",
                  ...
                  {SCHOOL_CLASSES.map((cls) => {
                    ...
                    return (
                      <tr key={cls}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <span
                            className={`badge ${pct >= 85 ? "badge-success" : "badge-warning"}`}
                          >
                            {pct}%
                          </span>
                        </td>
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Student ID:{" "}
                  <strong>{selectedFineForDetails.studentId}</strong> • Roll No:{" "}
                  <strong>{selectedFineForDetails.rollNumber}</strong>
                </div>
                <div
                  style={{
                    fontSize: "0.85rem",
                ...
                >
                  Class:{" "}
                  <strong>
                    {selectedFineForDetails.class} - Section{" "}
                    {selectedFineForDetails.section}
                  </strong>{" "}
                  • Parent: <strong>{selectedFineForDetails.parentName}</strong>{" "}
                  ({selectedFineForDetails.parentPhone})
                </div>
              </div>
            ...
            <div className="grid-2" style={{ gap: "14px" }}>
              ...
              >
                ...
                >
                  ...
                  <strong>
                    {formatCurrency(selectedFineForDetails.totalFee)}
                  </strong>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <strong style={{ color: "#ef4444" }}>
                    {selectedFineForDetails.dueDate}
                  </strong>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <strong style={{ color: "#10b981" }}>
                    {formatCurrency(selectedFineForDetails.paidAmount)}
                  </strong>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <strong style={{ color: "#f59e0b" }}>
                    {formatCurrency(selectedFineForDetails.pendingAmount)}
                  </strong>
                </div>
              </div>
              ...
              >
                ...
                >
                  ...
                  <strong
                    style={{
                      color:
                        selectedFineForDetails.lateDays > 0
                          ? "#ef4444"
                          : "#10b981",
                    }}
                  >
                ...
                >
                  <span style={{ color: "var(--text-secondary)" }}>
                    Billable Days (Late - Grace):
                  </span>
                  <strong>
                    {selectedFineForDetails.lateDaysAfterGrace || 0} Days
                  </strong>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <strong>
                    {fineSettings.fineType === "fixed"
                      ? formatCurrency(fineSettings.fixedAmount || 500)
                      : `${formatCurrency(fineSettings.fineAmount || 50)} / day`}
                  </strong>
                </div>
                <div
                  style={{
                ...
                >
                  <span style={{ color: "#8b5cf6", fontWeight: 700 }}>
                    Total Fine Outstanding:
                  </span>
                  <strong style={{ color: "#8b5cf6", fontSize: "1rem" }}>
                    {formatCurrency(selectedFineForDetails.fineAmount)}
                  </strong>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <strong style={{ color: "#ef4444" }}>
                    {selectedFineForDetails.lateDays}d
                  </strong>
                </div>
                ...
                >
                  ...
                  <strong style={{ color: "#dc2626" }}>
                    {formatCurrency(
                      selectedFineForDetails.totalOutstanding ||
                        selectedFineForDetails.pendingAmount,
                    )}
                  </strong>
                </div>
              </div>
            ...
            >
              <button
                className="btn btn-outline"
                onClick={() => setSelectedFineForDetails(null)}
              >
                Close
              </button>
              <button
      ...
      {isFineSettingsModalOpen && (
        ...
        >
          <form
            onSubmit={handleSaveFineSettings}
          ...
          >
            ...
            >
              Configure grace periods and automated late fee penalty rates.
              Fines calculate deterministically without duplicate increments.
            </p>
            ...
            <div>
              ...
              <select
                name="fineType"
                className="form-control"
                defaultValue={fineSettings.fineType || "per_day"}
              >
                <option value="per_day">
                  Per Day Rate (e.g. ₹50 / day late)
                </option>
                <option value="fixed">
                  Fixed Penalty Flat Fee (e.g. ₹500)
                </option>
              </select>
            </div>
            ...
            >
              <button
                type="button"
                className="btn btn-outline"
                onClick={() => setIsFineSettingsModalOpen(false)}
              >
                Cancel
              </button>
        ...
        >
          ...
          >
            ...
            >
              <div>
                <h3 style={{ fontSize: "1.25rem", fontWeight: 800, margin: 0 }}>
                  {selectedStudentForView.studentName}
                </h3>
                <div
                  style={{
                    fontSize: "0.85rem",
                ...
                >
                  Student ID:{" "}
                  <strong>{selectedStudentForView.studentId}</strong> • Roll No:{" "}
                  <strong>{selectedStudentForView.rollNumber}</strong>
                </div>
                <div
                ...
                >
                  Class:{" "}
                  <strong>
                    {selectedStudentForView.class} - Section{" "}
                    {selectedStudentForView.section}
                  </strong>{" "}
                  • Session: {selectedStudentForView.academicSession}
                </div>
              </div>
            ...
            {selectedStudentForView.status === "OVERDUE" && (
              ...
              >
                ...
                <span>
                  🔴 Payment overdue by {selectedStudentForView.lateDays} days
                  (Due Date: {selectedStudentForView.dueDate})
                </span>
              </div>
            )}
            ...
            <div className="grid-3" style={{ gap: "10px" }}>
              ...
              >
                ...
                >
                  {formatCurrency(selectedStudentForView.totalFee)}
                </div>
              </div>
              ...
              >
                ...
                >
                  {formatCurrency(
                    selectedStudentForView.totalOutstanding ||
                      selectedStudentForView.pendingAmount,
                  )}
                </div>
                {selectedStudentForView.fineAmount > 0 && (
                  <div style={{ fontSize: "0.7rem", color: "#ef4444" }}>
                    (Includes{" "}
                    {formatCurrency(selectedStudentForView.fineAmount)} Late
                    Fine)
                  </div>
                )}
              </div>
                    ...
                    <tr key={idx}>
                      ...
                      <td
                        style={{
                          textAlign: "right",
                          color:
                            b.pending > 0 ? "#f59e0b" : "var(--text-tertiary)",
                        }}
                      >
                      ...
                      <td>
                        <span
                          className={`badge ${b.status === "Paid" ? "badge-success" : b.status === "Partial" ? "badge-warning" : "badge-info"}`}
                        >
                          {b.status}
                        </span>
                      </td>
      ...
      {paymentModalData && (
        ...
        >
          ...
          >
            ...
            >
              <div>
                <strong>{paymentModalData.studentName}</strong> (ID:{" "}
                {paymentModalData.studentId})
                <div
                  style={{ fontSize: "0.8rem", color: "var(--text-tertiary)" }}
                >
                  Class: {paymentModalData.class} - Sec{" "}
                  {paymentModalData.section}
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                ...
                <strong style={{ color: "#ef4444", fontSize: "1.1rem" }}>
                  {formatCurrency(
                    paymentModalData.totalOutstanding ||
                      paymentModalData.pendingAmount,
                  )}
                </strong>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <strong style={{ color: "#6366f1", fontSize: "1.15rem" }}>
              ...
              >
                ...
                <div>
                  ...
                  <strong>
                    {selectedReceiptForPrint.class} - Section{" "}
                    {selectedReceiptForPrint.section}
                  </strong>
                </div>
                <div>
                  <span style={{ color: "#64748b" }}>Payment Date:</span>{" "}
                  <strong>
                    {selectedReceiptForPrint.paidDate ||
                ...
                <tbody>
                  <tr style={{ borderBottom: "1px solid #f1f5f9" }}>
                    <td style={{ padding: "10px 4px" }}>
                      <strong>
                        {selectedReceiptForPrint.feeType ||
                          selectedReceiptForPrint.feeHead ||
                          "Tuition Fee Payment"}
                      </strong>
                      <div style={{ fontSize: "0.8rem", color: "#64748b" }}>
              ...
              >
                <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>
                  * This is a computer generated receipt. Valid without physical
                  signature.
                </div>
